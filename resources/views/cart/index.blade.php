            ...
            <div class="w-full flex  flex-row justify-between items-center gap-10">
                <div class="w-2/4  flex flex-row gap-4 items-center">
                    <div class=" w-fit lg:w-24 h-24 bg-slate-200 flex justify-center items-center rounded-lg">
                        <img class="w-20 h-20" src="{{ Storage::url($item->product->photo) }}" alt="">
                    </div>
                    <div class="">
                ...
                <div class="w-1/4 h-fit flex justify-center items-center gap-4 md:gap-4">
                    ...
                    <form action="{{ route('cart.quantity') }}" method="POST" style="display:inline;">
                        ...
                        <button type="submit" name="button-min" class="text-gray-400 hover:text-white border border-gray-400 hover:bg-gray-600 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-2.5 py-1 text-center">-</button>
                    </form>
                    ...
                    <form action="{{ route('cart.quantity') }}" method="POST" style="display:inline;">
                        ...
                        <button type="submit" name="button-plus" class="text-gray-400 hover:text-white border border-gray-400 hover:bg-gray-600 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-2.5 py-1 text-center">+</button>
                    </form>
                </div>
                <p class="w-1/4 text-center hidden md:block font-medium text-gray-700"> Rp. {{ number_format($itemTotal, 0, ',', '.') }}</p>
            </div>
        @endforeach
       </div>
